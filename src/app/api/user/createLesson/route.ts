...
import { createS3Lesson } from "@/app/(main_page)/createCourses/components/actions/s3files/s3CreateLesson";
...
export async function POST(request: Request) {
    ...
    try {
        const result = await createS3Lesson(fileBuffer, folderName, fileName, name, aboutCourse, courseId);
        if (result) {
