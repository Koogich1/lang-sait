...
const createNewCourse = () => {
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    reader.onloadend = async () => {
      ...
      const folderName = `course_${user.id}_${data.name}`; // Указать имя папки
      const fileName = `oblozhka_${data.name}.jpg`; // Генерация имени файла

      try {
        await createS3photo(base64Data, folderName, fileName);
        console.log('Файл успешно загружен на S3');
        // Здесь вы можете дополнительно обработать успешную загрузку
      } catch (error) {
  ...
  return (
    <Dialog open={open}>
      <DialogTrigger
        className="px-4 py-[120px] flex flex-col items-center justify-center gap-3 h-20 border-2 border-dashed mt-5 hover:border-solid hover:border-purple-600 transition-all rounded-lg"
        onClick={() => {
          setOpen(true);
        }}
      >
