...
import { motion, useAnimation } from "framer-motion"; // Импортируем
...
const formatDateToFullMonth = (dateString: any) => {
  ...
  // Список названий месяцев с учетом склонений
  const months = [
...
const LessonForTeacher = ({ Teacher, user, userSubs, booking }: Props) => {
  ...
  // Убедитесь, что booking существует и не пуст, чтобы избежать ошибок
  useEffect(() => {
  ...
  if (!user || !booking) {
    ...
    return (
			<div className="flex gap-2 items-center text-lg font-medium text-gray-400">
				с <div className="text-white flex items-center justify-center py-1 px-3 cursor-pointer bg-blue-400 hover:bg-blue-500 transition-all rounded-lg">{startTime}</div> <FaArrowRight/> <div className="text-white flex cursor-pointer items-center justify-center py-1 px-3 bg-blue-400 hover:bg-blue-500 transition-all rounded-lg">{endTime}</div>
			</div>
		)
  ...
  return (
    <div className="px-2 py-3 flex flex-col justify-between h-[165px]">
      <div
        className="text-xl flex justify-between text-[#835BD2] font-semibold"
      >
        <h1>{currentBookingIndex === 0 ? "Ближайшее занятие:" : "Следующее занятие:"}</h1>
				<div className="flex gap-2 items-center">
					<div className="px-3 py-1 bg-[#835BD2] text-base rounded-lg cursor-pointer transition-all hover:bg-[#704eb5] text-white font-medium">
						Сегодня:
						{" " + formatDateToFullMonth(new Date())}
					</div>
				</div>
      </div>
      <motion.div animate={controls} initial={{ opacity: 0 }} className="flex items-center justify-center">
        <div className="flex gap-2 items-center border border-gray-100 shadow-md py-3 px-4 rounded-lg justify-center w-1/2">
          <div className="px-3 py-1 bg-[#835BD2] text-xl rounded-lg cursor-pointer transition-all hover:bg-[#704eb5] text-white font-medium">
						{formatDateToFullMonth(booking[currentBookingIndex].date.toJSON())}
					</div>
					<div>
						{formatTimeSlot(booking[currentBookingIndex].timeSlot)}
					</div>
        </div>
      </motion.div>
      {booking.length > 1 && (
