...
type Props = {
	...
	courseId: CustomCourse | undefined;
	customSetId: string | undefined;
	visov: () => void;
}

const Settings = ({openModal, setOpenModal, courseId, customSetId, visov} : Props) => {
			...
			<DialogContent>
				<DialogHeader>
					...
					<div className="flex gap-2 items-center text-xl font-light text-gray-500">
						<Image src={courseId?.photoUrl ? courseId?.photoUrl : ""} alt="logo" width={100} height={100} className="w-20 h-20 rounded-lg" />
						<h1>{courseId?.name}</h1>
					</div>
					</DialogTitle>
					<DialogDescription>
