...
export async function S3UpdateLesson(fileBuffer: Buffer, folderName: string, fileName: string, name: string, aboutLesson: string, lessonId: string) {
    ...
    try {
        ...
        try{
					updateLesson({ name: name, aboutLesson: aboutLesson, photoUrl: question, lessonId: lessonId});
        }catch(e){
            console.log(e)
        }
