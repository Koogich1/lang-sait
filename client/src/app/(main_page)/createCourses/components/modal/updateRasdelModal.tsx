...
import { rasdelId, User } from "@prisma/client";
import { currentUser } from "@/lib/auth";
import fetchRasdelById from "../actions/fethRasdelById";
...
const UpdateRasdelModal = ({ updateData, rasdelId }: Props) => {
  const [open, setOpen] = useState(false);
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [user, setUser] = useState<User | null>(null);
  const [rasdel, setRasdel] = useState<rasdelId | null>(null);
  ...
  useEffect(() => {
    const fetchRasdel = async () => {
      const data = await fetchRasdelById(rasdelId);
      if (data) {
        setRasdel(data);
        form.reset({
          name: data.name,
          aboutCourse: data.aboutRasdel,
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    if(file){
      ...
      formData.append("rasdelId", rasdelId)
      try{
        const response = await fetch('/api/user/updateRasdel', { // Путь к вашему API
          method: 'POST',
          body: formData,
       });
      ...
      setLoading(true);
      try {
        updateRasdel({rasdelId: rasdelId, name: name, aboutRasdel: data.aboutCourse})
        setOpen(false)
      } catch (e) {
        console.error(e);
      } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[420px]">
        ...
        <DialogHeader>
          <DialogTitle className="text-gray-600 text-3xl font-bold">Изменить раздел</DialogTitle>
        </DialogHeader>
...
export default UpdateRasdelModal;
