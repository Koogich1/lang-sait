...
const ConnectLettersModal = ({ test, updateVisov }: { test: Test, updateVisov: () => void }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <div className={`p-2 mt-3 bg-purple-200 hover:bg-[#835BD2] text-[#835BD2] hover:text-white transition-all text-lg absolute top-[-20px] right-0 rounded-lg cursor-pointer ${testInfo && testInfo.questionType === "AUDIOCHOOSE" ? "top-[-10rem]" : ""}`}>
            ...
            <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between my-3">
										<FormLabel className="w-1/3 text-base font-semibold text-gray-400 mt-1">Загаданное слово:</FormLabel>
										<FormControl className="w-2/3 text-lg">
											<Input placeholder="" {...field} className="flex justify-center text-lg items-center text-gray-400"/>
										</FormControl>
										<FormMessage />
									</FormItem>
                )}
...
export default ConnectLettersModal;
