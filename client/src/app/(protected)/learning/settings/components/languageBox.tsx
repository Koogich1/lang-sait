...
import { languageVariants, languagePrefers, Language } from "@prisma/client";
import findLanguages from '../actions/findLanguages';
import Image from 'next/image';
import Link from 'next/link';
import UpdateLanguageModal from './modal/updateLanguageModal';
...
const LanguageBox = () => {
  ...
  useEffect(() => {
    console.log("але, вызываю пользователя")
  ...
  useEffect(() => {
    const fetchLanguages = async () => {
      ...
      if (user) {
        const fetch = await findLanguages(user.teacherId);
  ...
  const onSubmit = async () => {
    if (language && level && prefers && user?.teacherId) {
      const teacherId = user.teacherId; // Убедитесь, что teacherId существует
    
      await addLanguage({ language: language as languageVariants, level, prefers: prefers as languagePrefers, teacherId, userId: user.id });
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger onClick={() => { 
          ...
          <div className="flex items-center justify-center w-[380px] h-12 gap-3 bg-purple-100 border-2 border-[#835BD2] hover:bg-[#835BD2] text-[#835BD2] hover:text-white rounded-xl border-dashed hover:border-solid transition-all cursor-pointer">
            <div><Image width={1000} height={1000} src={`https://storage.yandexcloud.net/langschoolacynberg/images/flags/English.png`} alt="flag" className='w-9 h-7 object-cover rounded-sm' /></div>
          ...
          <DialogHeader>
            ...
            {languageChoose ? (
                ...
                <div ref={contentRef} className='w-full mt-3'>
                  {language === "China" ? (
                    <div className='flex gap-3 w-full flex-col'>
                      ...
                      <div>
                      <h1 className='text-lg text-gray-400 font-medium'>Уровень преподавания:</h1>
                      ...
                      <h1 className='text-lg text-gray-400 font-medium'>Предпочитаемый возраст:</h1>
                      <Select onValueChange={(value) => setPrefers(value as languagePrefers)}>
                        <SelectTrigger className="w-full">
                        <SelectValue
              ...
              <div className='flex gap-3 mt-3'>
                <Button className='w-1/2' variant={level && prefers ? "violetSelect" : "shadow2"}
                onClick={() => {
                  if(level && prefers){
                    onSubmit()
                    fetchLanguages()
  ...
  return (
    <div className='w-full'>
      <div ref={contentRef} className="overflow-hidden transition-transform duration-500 grid grid-cols-1 lg:grid-cols-2 gap-3 w-full">
        {languages.map((data) => (
          ...
          >
            {data.language === "China" ? 
            <div className="bg-[#f20520] p-4 overflow-hidden transition-all relative min-h-[120px] hover:opacity-70 w-full flex flex-col justify-between rounded-xl shadow-lg">
            ...
            <div className="bg-amber-500 w-full p-4 overflow-hidden transition-all hover:opacity-70 relative min-h-[120px] flex flex-col justify-between rounded-xl shadow-lg">
            ...
            <div className="bg-[#ffe2ef] w-full p-4 overflow-hidden transition-all hover:opacity-70 relative min-h-[120px] flex flex-col justify-between rounded-xl">
        ...
        <DialogTrigger onClick={() => { 
          ...
          <div className="flex items-center justify-center py-2 h-20 gap-3 bg-purple-100 border-2 border-[#835BD2] hover:bg-[#835BD2] text-[#835BD2] hover:text-white rounded-xl border-dashed hover:border-solid transition-all cursor-pointer">
            <div><Image width={1000} height={1000} src={`https://storage.yandexcloud.net/langschoolacynberg/images/flags/English.png`} alt="flag" className='w-9 h-7 object-cover rounded-sm' /></div>
            <p className='font-medium'>Добавить еще языки!</p>
          </div>
        </DialogTrigger>
        <DialogContent className='overflow-hidden max-w-[550px]'>
          <DialogHeader>
      ...
      <UpdateLanguageModal language={activeLanguage} openModal={updateMDL} setOpenModal={setUpdateModal} visov={() => fetchLanguages()}/>
    </div>
  );
};
