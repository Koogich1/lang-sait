...
const Page = () => {
  ...
  return (
    <div className='w-full bg-white rounded-lg shadow-lg mt-5 p-5 flex flex-col text-gray-400 transition-all'>
      <h1 className='text-2xl font-medium text-[#835BD2]'>Материалы</h1>
      ...
      <div className={`transition-opacity duration-300 ${isTransitioning ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        {active === "My Courses" ?
            ...
            {lessons?.length === 0 || lessons === null ?
              ...
              <div className='w-full min-h-[60vh] px-0 flex flex-col items-center text-gray-500'>
                ...
                <div className='flex w-full gap-5 pt-5'>
                  <Input
                    className='w-4/5 h-10 placeholder:text-gray-300 text-base'
                ...
                <div className='grid grid-cols-5 gap-5 w-full mt-5'>
                  {sortedLessons.map((data: any) => (
                    <Link href={`/learning/materials/${data.id}`} key={data.id} className='flex cursor-default items-center justify-center'>
                      <div className='w-[175px] h-[295px] rounded-lg overflow-hidden relative flex justify-center hover:scale-105 hover:shadow-lg transition-all duration-300 cursor-pointer'>
                        {data.imageSrc === "" ? (
                  ...
                  <div className='flex items-center justify-center'>
                    <Button ref={boxRef} id='box' variant={"violetSelect"} className='w-[175px] h-[295px] border-2 border-[#835BD2] bg-white border-dotted text-white hover:bg-[#835BD2] shadow-md' onClick={() => {
                      setOpen(true);
                    }}>
          ...
          <div className='w-full h-full justify-center items-start'>
            ...
            <div className='grid grid-cols-5 gap-5 justify-center w-full items-center mt-6'>
