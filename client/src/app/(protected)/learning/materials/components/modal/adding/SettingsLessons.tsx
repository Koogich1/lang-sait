...
const SettingsLesson = ({openModal, setOpenModal, lesson, customSetId, visov} : Props) => {
			...
			<DialogContent>
				<DialogHeader>
					...
					<div className="flex gap-2 items-center text-xl font-light text-gray-500">
						<Image src={lesson?.photoUrl ? lesson?.photoUrl : ""} alt="logo" width={100} height={100} className="w-20 h-20 rounded-lg object-cover border shadow-md border-gray-200" />
						<h1>{lesson?.name}</h1>
					</div>
					</DialogTitle>
					<DialogDescription>
