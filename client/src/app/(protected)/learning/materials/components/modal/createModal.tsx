  ...
  const [loading, setLoading] = useState(false)
	...
	return(
		<Dialog open={openModal} onOpenChange={setOpenModal}>
			<DialogContent>
			<div
          className="w-6 h-6 bg-red-400 rounded-sm cursor-pointer hover:bg-red-500 absolute right-0 top-0 m-7"
          onClick={() => setOpenModal(false)}
        >
          <IoCloseOutline className="text-2xl text-white" />
        </div>
        <DialogHeader>
          <DialogTitle className="text-[#835BD2] text-2xl font-semibold">Создание уникального урока</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              render={({ field }) => (
                <FormItem className="grid grid-cols-2 justify-center items-center space-y-0 gap-3 ml-0">
                  ...
                  <div className="flex flex-col w-full max-w-sm items-center gap-1.5">
                    <Label htmlFor="picture" className="text-lg text-[#835BD2] font-semibold">Обложка</Label>
                    <Input
                      id="picture"
                      type="file"
                    ...
                    <Button
                      ...
                      className="font-semibold text-lg bg-[#835BD2] px-5 hover:bg-[#6747a9]"
                    >
                      Загрузить
                    </Button>
                  </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex justify-center items-center space-y-0 gap-5 ml-0 mt-5">
                  <Label htmlFor="name" className="text-base text-[#835BD2] font-semibold w-2/5">Название</Label>
                  <Input
                    {...field}
                    placeholder="Введите название"
                    className="w-3/5 h-[40px] placeholder:text-gray-300 text-gray-600"
                  />
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex justify-center items-center space-y-0 gap-5 ml-0 mt-5">
                  <Label htmlFor="aboutCourse" className="text-base text-[#835BD2] font-semibold w-2/5">Описание урока</Label>
                  <Textarea
                    {...field}
