...
const CreateRasdelModal = ({openModal, setOpenModal, visov, courseId}: Props) => {
	...
	return (
		<Dialog open={openModal} onOpenChange={setOpenModal}>
			<DialogContent>
				<DialogHeader>
					<DialogTitle><p className='text-2xl font-light text-gray-500'>Создание раздела</p></DialogTitle>
				</DialogHeader>
				<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className="">
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormControl>
									<Input placeholder="Введите название раздела" className='w-full placeholder:text-gray-300 text-gray-600' {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
