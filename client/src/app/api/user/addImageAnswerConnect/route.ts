import { s3UpdateAnswerImageConnect } from "@/app/(main_page)/createCourses/components/actions/s3files/s3UpdateAnswerImageConnect";
...
export async function POST(request: Request) {
    ...
    const fileName = `answer_${currOptionId}_${Math.floor(Math.random() * 1000000)}`; 
    ...
    const fileBuffer = Buffer.from(arrayBuffer); // Создаем Buffer из ArrayBuffer
		...
		try {
			const result = await s3UpdateAnswerImageConnect(fileBuffer, folderName, fileName, testId, currOptionId);
			if (result) {
