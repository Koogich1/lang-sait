...
import { S3UpdateLesson } from "@/app/(main_page)/createCourses/components/actions/s3files/s3UpdateLesson";
...
export async function POST(request: Request) {
    ...
    const name = data.get("name") as string;
		const aboutLesson = data.get('aboutLesson') as string;
    ...
    try {
        const result = await S3UpdateLesson(fileBuffer, folderName, fileName, name, aboutLesson, lessonId);
        if (result) {
